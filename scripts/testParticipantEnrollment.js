...
async function testParticipantEnrollment() {
  ...
  try {
    ...
    for (let i = 0; i < numParticipants; i++) {
      ...
      const totalWeight = testTrial.randomizationRatio.reduce(
      ...
      console.log(
        `👤 Enrolled ${participantCode} → Group ${assignedGroup} (${testTrial.arms[assignedGroup].name})`
      );
    }
    ...
    testTrial.arms.forEach((arm, index) => {
      ...
      console.log(
        `   ${arm.name}: ${count}/${numParticipants} (${percentage}%)`
      );
    });

    // Test blinded data retrieval
    console.log("\n🔒 Testing Blinded Data Retrieval:");
    const blindedParticipants = await Participant.find({
      trialId: testTrial._id,
    })
    ...
    console.log(
      `   Retrieved ${blindedParticipants.length} participants (assignedGroup hidden)`
    );
